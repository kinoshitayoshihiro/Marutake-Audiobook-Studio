...
OUT_DIR="$SCRIPT_DIR/ninjo_channel_report"
VENV_DIR="${VENV_DIR:-$SCRIPT_DIR/.venv}"
PYTHON_BIN="$VENV_DIR/bin/python"
TOKEN_FILE="token_ninjo_channel.json"
...
log "Starting ninjo channel report refresh"
log "Target window: $START_DATE to $END_DATE"
log "Authenticate with the ninjo channel owner account when the Google OAuth screen appears"
...
if [[ $(wc -l < "$OUT_DIR/youtube_video_report_last_90_days_all_videos.csv") -le 1 ]]; then
  log "No videos were returned for this account in the requested window. Stopping before downstream analysis."
  exit 2
fi
